    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <?= $form->field($model, 'user_id')->hiddenInput(['class' => 'my-0'])->label(false); ?>
                    <?= $form->field($model, 'scenario')->hiddenInput(['class' => 'my-0'])->label(false); ?>
                        <label>&nbsp;</label>
                    <?php if(!Yii::$app->user->isGuest && ($model->getScenario() == $model::SCENARIO_EmployeeEditClient || Yii::$app->user->getId() == $model->user_id)): ?>
                        <div class="row">
                            <div class="col-md-8">
                                <?= Html::submitButton('Сохранить изменения', ['class' => 'btn custom-btn sm primary text-uppercase w-100'])?>
                            </div>
                            <div class="col-md-4">
                                <button type="button" class="btn custom-btn gray sm text-uppercase w-100" data-dismiss="modal">Отмена</button>
                            </div>
                        </div>
